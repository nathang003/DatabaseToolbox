CREATE PROCEDURE [dbo].[spScrapes_GetAllUnassigned]
AS
BEGIN
	SET NOCOUNT ON;

	UPDATE dbo.Scrapes
	SET ScrapeStatusId = 2,
		ScrapeWorkerId = NULL
	WHERE ScrapeId in 
		(
			SELECT ScrapeId
			FROM dbo.scrapes
			WHERE ScrapeWorkerId IS NOT NULL
			AND ScrapeStatusId BETWEEN 3 AND 4
			AND DATEDIFF(DAY, CreatedDate, GETUTCDATE()) > 0
		)
